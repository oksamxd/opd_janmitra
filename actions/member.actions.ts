...
export class MemberActions {
  ...
  async verifyMember(memberId: string) {
    ...
    if (member) {
      await this.audit.log('MEMBER_ACTIONS', 'VERIFY_MEMBER', 'members', memberId, { found: true });
      this.logger.log(`✅ Member verified: ${member.full_name} (${memberId})`);
    } else {
      await this.audit.log('MEMBER_ACTIONS', 'VERIFY_MEMBER', 'members', memberId, { found: false });
  ...
  async ensureMedicalRecord(memberId: string) {
    ...
    if (member.medical_record_id) {
      this.logger.log(`Medical record already exists: ${member.medical_record_id}`);
      return { memberId, medicalRecordId: member.medical_record_id, isNew: false };
    }
    ...
    await this.audit.log('MEMBER_ACTIONS', 'CREATE_MEDICAL_RECORD', 'members', memberId, { medicalRecordId: recordId });
   ...
   */
  async createMemberWithMedicalRecord(data: { full_name: string; email?: string; phone?: string; age?: number | null; gender?: string; height?: string; weight?: string; blood_group?: string; }) {
    ...
    await this.audit.log('MEMBER_ACTIONS', 'CREATE_MEMBER_WITH_MR', 'members', member.member_id, {
      full_name: data.full_name,
      record_id: member.medical_record?.record_id,
    });
    this.logger.log(`✅ Member AND Medical Record created: ${member.full_name} (${member.member_id})`);
